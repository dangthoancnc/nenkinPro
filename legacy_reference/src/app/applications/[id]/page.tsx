    ...
    <div className="max-w-5xl mx-auto space-y-6 pb-12">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          <Link href="/applications" className="p-2 text-slate-400 hover:text-slate-900 bg-white rounded-lg border border-slate-200 shadow-sm transition-colors">
            <ArrowLeft className="w-5 h-5" />
          </Link>
          <div>
            <h1 className="text-2xl font-bold text-slate-900">Chi tiết Hồ sơ</h1>
            <p className="text-sm text-slate-500">
              Khách hàng: <span className="font-medium text-slate-900">{appData.customer.fullName}</span>
            </p>
            <div className="flex items-center gap-2 mt-1">
              <span className="text-xs font-mono bg-indigo-50 text-indigo-700 px-2 py-0.5 rounded border border-indigo-200">Mã hồ sơ: {appData.customer.code}</span>
              {appData.customer.cardNumber && (
                <span className="text-xs font-mono bg-emerald-50 text-emerald-700 px-2 py-0.5 rounded border border-emerald-200">Thẻ NK: {appData.customer.cardNumber}</span>
              )}
            </div>
          </div>
        </div>
        <div className="flex items-center gap-3">
          <Link 
            ...
            target="_blank"
            className="inline-flex items-center gap-2 px-4 py-2.5 bg-white border border-slate-200 text-slate-700 font-medium rounded-lg hover:bg-slate-50 transition-all shadow-sm"
          >
          ...
          <div className="flex gap-2">
            <button 
              ...
              className="inline-flex items-center gap-2 px-4 py-2.5 bg-indigo-50 border border-indigo-200 text-indigo-700 font-medium rounded-lg hover:bg-indigo-100 transition-all shadow-sm"
            >
              <Printer className="w-4 h-4 text-indigo-500" />
              <span className="hidden sm:inline">Đơn Xin Lần 1</span>
            </button>
          </div>
          <button 
            onClick={handleSave}
          ...
          {STATUS_STEPS.map((step, index) => {
            ...
            return (
              <React.Fragment key={step.id}>
                ...
                >
                  <div className={`w-10 h-10 rounded-full flex items-center justify-center border-2 transition-all duration-300 ${isActive ? 'bg-primary border-primary text-white scale-110 shadow-lg shadow-primary/30' : isCompleted ? 'bg-emerald-500 border-emerald-500 text-white' : 'bg-white border-slate-300 text-slate-400'}`}>
                    {isCompleted && !isActive ? <CheckCircle className="w-5 h-5" /> : <span className="text-sm font-bold">{index + 1}</span>}
                  </div>
                  <span className={`text-xs font-semibold ${isActive ? 'text-primary' : isCompleted ? 'text-emerald-600' : 'text-slate-500'}`}>{step.label}</span>
                </div>
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm mt-6">
            ...
            <div className="space-y-3">
              <button 
                onClick={() => handleGenerateDoc('脱退一時金請求書.docx')}
                disabled={generatingDoc}
                className="w-full flex items-center justify-between px-4 py-3 bg-slate-50 hover:bg-blue-50 border border-slate-200 hover:border-blue-200 rounded-xl transition-colors disabled:opacity-50"
              >
                <div className="flex flex-col text-left">
                  <span className="font-medium text-slate-800">Đơn xin Rút Nenkin (Lần 1)</span>
                  <span className="text-xs text-slate-500">Định dạng .docx</span>
                </div>
                {generatingDoc ? <div className="w-4 h-4 border-2 border-blue-500 border-t-transparent rounded-full animate-spin"/> : <FileText className="w-5 h-5 text-slate-400" />}
              </button>
              <button 
                onClick={() => handleGenerateDoc('委 任 状.docx')}
                disabled={generatingDoc}
                className="w-full flex items-center justify-between px-4 py-3 bg-slate-50 hover:bg-blue-50 border border-slate-200 hover:border-blue-200 rounded-xl transition-colors disabled:opacity-50"
              >
                <div className="flex flex-col text-left">
                  <span className="font-medium text-slate-800">Uỷ Nhiệm Thư (Lần 2)</span>
                  <span className="text-xs text-slate-500">Định dạng .docx</span>
                </div>
                {generatingDoc ? <div className="w-4 h-4 border-2 border-blue-500 border-t-transparent rounded-full animate-spin"/> : <FileText className="w-5 h-5 text-slate-400" />}
              </button>
