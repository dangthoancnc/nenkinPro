...
// Dummy implementation of PrintTab and ApplicationDetailsTab until we port them
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col overflow-hidden h-screen">
        ...
        <div className="max-w-screen-2xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <nav className="flex gap-6 mt-2 overflow-x-auto">
            ...
            <button 
              ...
              className={`border-b-2 font-medium py-3 px-2 flex items-center gap-2 whitespace-nowrap ${activeTab === 'print' ? 'border-indigo-600 text-indigo-600' : 'border-transparent text-slate-500 hover:text-slate-700'}`}>
      ...
      <main className="flex-1 overflow-hidden relative">
        {activeTab === 'data_entry' && (
          <div className="h-full flex flex-col lg:flex-row gap-6 p-4 sm:p-6 lg:p-8 max-w-screen-2xl mx-auto">
            
            {/* LEFT COLUMN: Documents & OCR (Sticky/Fixed View) */}
            <div className="lg:w-5/12 flex flex-col gap-4 bg-slate-50 h-full rounded-xl border border-slate-200 overflow-hidden">
              <div className="p-4 border-b border-slate-200 bg-white flex justify-between items-center shrink-0">
                <h2 className="font-bold text-slate-800">Tài liệu & OCR</h2>
                {selectedDocKey && (
                  <Button variant="ghost" size="sm" onClick={() => setSelectedDocKey(null)} className="h-8 text-slate-500 hover:text-slate-800">
              ...
              <div className="space-y-8 bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
                ...
                <section>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="p-4 bg-emerald-50/30 rounded-xl border border-emerald-100">
                      <h3 className="text-sm font-semibold text-emerald-900 mb-3">Địa chỉ tại Việt Nam</h3>
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <section>
                  <div className="flex items-center justify-between border-b border-slate-200 pb-2 mb-4">
                    <h2 className="text-lg font-bold text-slate-800">4. Lịch sử làm việc</h2>
                    <Button 
        ...
        {activeTab === 'app_details' && <AppDetailsTab customer={customer} />}
        {activeTab === 'print' && <PrintTab customerId={customer.id} />}
...
const InputField = ({ label, type = 'text', value, onChange, placeholder }: any) => (
  <div className="space-y-1">
    ...
    <input 
      ...
      className="w-full border-slate-300 rounded-lg text-sm bg-white focus:ring-indigo-500 focus:border-indigo-500" 
    />
  </div>
);
  ...
  return (
    <div className="space-y-1 relative">
        ...
        <div className="flex space-x-1">
          {onSyncFromAI && (
            <button
              ...
              className={`flex items-center justify-center w-6 h-6 rounded-md transition-all ${isVerified ? 'bg-slate-100 text-slate-400 cursor-not-allowed opacity-50' : 'bg-indigo-50 text-indigo-600 hover:bg-indigo-100'}`}
            >
      ...
      <input 
        ...
        onChange={onChange} 
        placeholder={placeholder}
        className={`w-full border-slate-300 rounded-lg text-sm bg-white focus:ring-indigo-500 focus:border-indigo-500 transition-colors ${hasError ? 'border-red-500 ring-1 ring-red-500 bg-red-50' : ''} ${isVerified ? 'border-emerald-300 bg-emerald-50/20 text-emerald-900 font-medium' : ''}`} 
      />
    </div>
  );
};
