          ...
          {menuItems.map((item) => {
            ...
            return (
              <li key={item.name}>
                <Link
                  href={item.href}
                  className={`flex items-center ${isOpen ? 'gap-3 px-3' : 'justify-center'} py-2.5 rounded-md hover:bg-teal-800/50 hover:text-white transition-all duration-200 group relative overflow-hidden`}
                  title={!isOpen ? item.name : undefined}
                >
                  <Icon className="w-5 h-5 text-teal-400 group-hover:text-teal-200 transition-colors duration-200 shrink-0" />
                  {isOpen && <span className="font-medium text-sm whitespace-nowrap text-teal-100 group-hover:text-white">{item.name}</span>}
                </Link>
              </li>
            );
          })}
