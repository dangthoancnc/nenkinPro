...
- post_1 to post_7
- address_jp (or address)
- rep_fullName
- rep_fullName_kata
- rep_address
- rep_post_1 to rep_post_7
- departure_y_1 to departure_y_4
- departure_m_1 to departure_m_2
- departure_d_1 to departure_d_2
- doc_date_era_jp
- doc_date_era_yr
- doc_date_m
- doc_date_d
...
async function run() {
  console.log('Loading nouzeikanrinin.pdf...');
  const pdfPath = path.join(process.cwd(), 'public', 'forms', 'nouzeikanrinin.pdf');
  ...
  const prompt = `
  You are an expert PDF parser. I am providing you with a Japanese government PDF form (Tax Representative Declaration / 納税管理人届出書).
  ...
  try {
    ...
    const configPath = path.join(process.cwd(), 'src', 'lib', 'configs', 'form3_config.ts');
    fs.mkdirSync(path.dirname(configPath), { recursive: true });
    
    const tsCode = `import { PdfMappingConfig } from '../pdfGenerator';\n\nexport const FORM3_COORDINATES: PdfMappingConfig = ${text};`;
    fs.writeFileSync(configPath, tsCode);
    console.log('Successfully generated coordinates to src/lib/configs/form3_config.ts');
  } catch (error: any) {
